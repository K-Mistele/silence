...
func (d *Datagram) Marshall() ([]byte, error) {
	...
	flagByte = 0x00
	if d.Flags.SYN {
		flagByte = flagByte ^ 128
	} // 10000000
	if d.Flags.ACK {
		flagByte = flagByte ^ 64
	} // 01000000
	if d.Flags.FIN {
		flagByte = flagByte ^ 32
	} // 00100000
	if d.Flags.ERR_RST {
		flagByte = flagByte ^ 16
	} // 00010000
	if d.Flags.IsFragment {
		flagByte = flagByte ^ 8
	} // 00001000
	if d.Flags.Retransmit {
		flagByte = flagByte ^ 4
	} // 00000100
	if d.Flags.Reserved1 {
		flagByte = flagByte ^ 2
	} // 00000010
	if d.Flags.Reserved2 {
		flagByte = flagByte ^ 1
	} // 00000001
